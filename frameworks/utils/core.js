...
SCUDS.Sha1 = {
  ...
  sha1Hash: function(msg) {
    var i, j, t, s, T, 
    rotLeft = SCUDS.Sha1.rotateLeftChar,
    seqSwitch = SCUDS.Sha1.sequenceSwitch;
    var W = []; var a, b, c, d, e;
    // constants [§4.2.1]
    ...
    for (i=0; i<N; i++) {
        ...
        for (t=16; t<80; t++) W[t] = rotLeft(W[t-3] ^ W[t-8] ^ W[t-14] ^ W[t-16], 1);
        ...
        for (t=0; t<80; t++) {
            ...
            T = (rotLeft(a,5) + seqSwitch(s,b,c,d) + e + K[s] + W[t]) & 0xffffffff;
            e = d;
            d = c;
            c = rotLeft(b, 30);
            b = a;
